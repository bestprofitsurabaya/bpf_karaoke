...
async def get_current_user(authorization: Optional[str] = Header(None)) -> str:
    """
    Extract username dari access token (JWT tipe 'access').
    Fix: temp token TIDAK diterima di endpoint biasa, sehingga user yang
    belum mengganti password tidak bisa mengakses API terproteksi.
    """
    ...
    try:
        ...
        # Hanya access token yang boleh mengakses endpoint terproteksi
        if payload.get("type", "access") == "temp":
            raise HTTPException(
                status_code=401,
                detail="Anda harus mengganti password sebelum mengakses fitur ini",
            )
        ...
        raise HTTPException(status_code=401, detail="Session expired atau token tidak valid")


async def get_temp_token_user(authorization: Optional[str] = Header(None)) -> str:
    """
    Khusus endpoint force-change-password: hanya menerima temp token
    yang dikeluarkan saat login (ISO 27001 A.9.2.4).
    """
    if not authorization:
        raise HTTPException(status_code=401, detail="Token tidak ditemukan")
    ...
    try:
        ...
        if payload.get("type") != "temp":
            raise HTTPException(status_code=401, detail="Token tidak valid untuk aksi ini")
        username = payload.get("sub")
        if not username:
            raise HTTPException(status_code=401, detail="Token tidak valid")
        return username
...
async def force_change_password(
    ...
    current_user: str = Depends(get_temp_token_user)
):
    """
